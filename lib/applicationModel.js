...
class ApplicationModel {
  ...
  getModelClass({schema, schemaName}) {
    ...
    if (!this.isFinishedSettingClassNames) {
      Object.values(this.topLevelSchemasByParserSchemaId).forEach(topLevelSchema => {
        this.setupClassNames(topLevelSchema, "");
      });
    }
    this.isFinishedSettingClassNames = true;
    ...
    if (!modelClass) {
      modelClass = this.modelClassMap[schemaName] || this.modelClassMap[_.camelCase(schemaName)] || this.modelClassMap[schema?.$id()];
    }
  ...
  /**
   * Go from the top down of each top level schema, find anonymous schemas by x-parser-schema-id,
   * and attempt to apply a name to objects. Some may already have names if the schema is well defined.
   * @param {JSONSchema} schema 
   * @param {String} name 
   */
  setupClassNames(schema, name) {
    const props = schema.properties;
    if (props) {
      const schemaId = schema["x-parser-schema-id"];
      // Top level schema won't be anonymous
      if (schemaId && schema.type === "object" && this.isAnonymousSchema(schemaId)) {
        this.modelClassMap[schemaId].setClassName(name);
      }

      const propNames = Object.keys(props);
      propNames.forEach(propName => {
        const schemaObject = props[propName];
        const schemaId = schemaObject["x-parser-schema-id"];
        if (schemaId && schemaObject.type === "object" && this.isAnonymousSchema(schemaId)) {
          this.modelClassMap[schemaId].setClassName(propName);
        } else if (schemaObject.type === "array") {
          this.setupClassNames(schemaObject, propName);
        }
      });
    }
    ...
    if (items) {
      ...
      if (items.type === "object") {
        this.setupClassNames(items, name);
      }
	  }
  }
